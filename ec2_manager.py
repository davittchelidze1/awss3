...
def setup_security_group(ec2_client, vpc_id, sg_name, ssh_ip):
    ...
    try:
        response = ec2_client.create_security_group(
            GroupName=sg_name,
            Description="CLI created SG for SSH access",
            VpcId=vpc_id
        )
...
def wait_and_check_ssh(ip_address, port=22, timeout=60):
    """დაელოდება და შეამოწმებს SSH (22) პორტზე კავშირის ხელმისაწვდომობას socket-ის გამოყენებით"""
    print(f"პორტ 22-ზე კავშირის შემოწმება (IP: {ip_address})...")
    start_time = time.time()
    
    while True:
        try:
            with socket.create_connection((ip_address, port), timeout=3) as sock:
                print("✅ SSH პორტი (22) ღიაა და კავშირი დადასტურებულია.")
                return True
        except (socket.timeout, ConnectionRefusedError, OSError):
            if time.time() - start_time > timeout:
                print("❌ დრო ამოიწურა! 22-ე პორტთან დაკავშირება ვერ მოხერხდა.")
                return False
            time.sleep(5)

def main():
    parser = argparse.ArgumentParser(description="AWS EC2 Instance CLI Manager")
